<template>
   <div>
    ...
    <v-data-table :headers="headers" :items="items" class="elevation-1 mt-4">
      ...
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Models List</v-toolbar-title>
          <v-divider class="mx-4" inset vertical></v-divider>
          <v-spacer></v-spacer>
          <v-dialog  v-model="dialog" max-width="500px">
            <template v-slot:activator="{ on, attrs }">
              <v-btn color="primary" dark class="mb-2" v-bind="attrs" v-on="on"> New Model </v-btn>
            </template>
                    ...
                    <v-row>
                      <v-col cols="12" sm="12" md="12">
                          ...
                          <validation-provider name="make" rules="required" v-slot="{errors}">
                            <v-select v-model="editedItem.make" :items="makes" item-text="name" item-value="id" :error-messages="errors" label="Make"></v-select>
                          </validation-provider>
                      </v-col>
                    </v-row>
                  </v-container>
          ...
          <v-dialog v-model="dialogDelete" max-width="520px">
            <v-card>
              <v-card-title class="headline">Are you sure you want to delete this model?</v-card-title>
              <v-card-actions>
                <v-spacer></v-spacer>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Make', sortable: false, value: 'make.name'},
    ...
    makes:[],
    editedIndex: -1,
    editedItem: { id: 0, name: '', make: null },
    defaultItem: { id: 0, name: '', make: null },
  }),
  ...
  methods: {
    ...
    updateItem(){
      ...
      .then((res)=>{
        ...
        this.showAlert('success',`The model with id #${res.data.id} was updated.`);
      })
      .catch(()=>{
    ...
    deleteItem(){
      ...
      .then(()=>{
        ...
        this.closeDelete();
        this.showAlert('success',`The model has been removed.`);
      })
      .catch((err)=>{
    ...
    editItem (item) {
      ...
      this.editedItem = Object.assign({}, { id: item.id, name: item.name, make: item.main_make_id })
      this.dialog = true
    },
